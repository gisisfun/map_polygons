...
class Tiles():
    ...
    def to_shp_file(self,GArray,fName):
        #tabular_list = []
        w = shapefile.Writer('{sfPath}{slash}{f_name}'.format(f_name = fName, slash = self.Slash, sfPath = self.shapefilesPath)) # , shapeType=3)
        #setup columns
        ...
        w.close()    
        # create the PRJ file
        prj = open('{sfPath}{slash}{f_name}.prj'.format(f_name = fName, slash = self.Slash, sfPath = self.shapefilesPath), "w")
        epsg = 'GEOGCS["WGS 84",'
        epsg += 'DATUM["WGS_1984",'
