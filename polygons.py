...
def write_vrt_tabular_file(f_name):
    ...
    myfile = open('vrt{slash}{fname}.vrt'.format(fname = f_name.replace(' ', '_'), slash=slash), 'w') #open file for writing geojson layer
    myfile.write(vrt_content) #write vrt layer to open file
...
def write_vrt_file(f_name, shape, ext, ext_label):
    ...
    myfile = open('vrt{slash}{fname}_layer_{extlabel}.vrt'.format(slash = slash, fname = f_name, extlabel = ext_label.replace(' ', '_')), 'w') #open file for writing geojson layer
    myfile.write(vrt_content) #write vrt layer to open file
    myfile.close() #close file
    #to check: ogrinfo -ro -al box_106km_layer.vrt
...
##                    len_radial = geodesic(start,end).km
        ...
        myfile = open('geojson{slash}{outfile}_layer.json'.format(outfile = outfile, slash = slash), 'w') #open file for writing geojson layer in geojson format
        ...
        myfile = open('metadata{slash}{outfile}_metadata.json'.format(outfile = outfile, slash = slash), 'w') #open file for writing geojson layer
        myfile.write(str(json.dumps(layer_dict))) #write geojson layer to open file
        myfile.close() #close file
