...
def write_vrt_tabular_file(f_name):
    ...
    print('\n tabular definition in vrt format for csv file to written \
    to file: {0}.vrt'.format(f_name))
    myfile = open('vrt{slash}{fname}.vrt'.format(fname=f_name
    .replace(' ', '_'), slash=slash), 'w')
...
def write_vrt_file(f_name, shape, ext, ext_label):
    ...
    print('\n {0} vrt for {1} file to written to file: {2}_layer_{3}.vrt'
    .format(shape, ext_label, f_name, ext))
    myfile = open('vrt{slash}{fname}_layer_{extlabel}.vrt'
    .format(slash=slash, fname=f_name, extlabel=ext_label
    .replace(' ', '_')), 'w')  # open file for writing geojson layer
    myfile.write(vrt_content)  # write vrt layer to open file
    myfile.close()  # close file
